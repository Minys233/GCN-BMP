...
class GraphConvPredictorForPair(chainer.Chain):
    ...
    # def mediate_output(self, atoms_1, adjs_1, atoms_2, adjs_2):
    #     if self.xp == cuda.cupy:
    #         atoms_1 = cuda.to_gpu(atoms_1)
    #         adjs_1 = cuda.to_gpu(adjs_1)
    #         atoms_2 = cuda.to_gpu(atoms_2)
    #         adjs_2 = cuda.to_gpu(adjs_2)
    #
    #     # genrate mediate layer outputs
    #     self.graph_conv(atoms_1, adjs_1)
    #     self.graph_conv(atoms_2, adjs_2)
    #     e1_list = self.graph_conv.encode_layer_outputs()
    #     e2_list = self.graph_conv.encode_layer_outputs()
...
class MyEvaluator(object):
    ...
    def precision(self, y_total, t_total):
        # --- ignore labels if specified ---
        if self.ignore_labels:
            valid_ind = numpy.in1d(t_total, self.ignore_labels, invert=True)
            y_total = y_total[valid_ind]
        ...
        precision = metrics.precision_score(y_true=t_total, y_pred=y_total)
        # modify
        # precision = metrics.precision_score(y_true=t_total, y_pred=y_total, pos_label=0)
        return precision

    def recall(self, y_total, t_total):
        # --- ignore labels if specified ---
        if self.ignore_labels:
            valid_ind = numpy.in1d(t_total, self.ignore_labels, invert=True)
            y_total = y_total[valid_ind]
        ...
        y_total = numpy.where(y_total > 0, 1, 0)
        y_total = numpy.asarray(y_total, dtype=numpy.int32)
        recall = metrics.recall_score(y_true=t_total, y_pred=y_total)
        # modify
        # recall = metrics.recall_score(y_true=t_total, y_pred=y_total, pos_label=0)
        return recall
...
def main():
    ...
    perf_dict = dict()
    for metric in ['roc_auc', 'prc_auc', 'accuracy', 'precision', 'recall', 'f1']:
    ...
    calculator = IntegratedGradientsCalculator(
        model.predictor, steps=5, eval_fun=eval_func, target_extractor=VariableMonitorLinkHook(model.predictor.graph_conv.embed, timing='post'),
        device=args.gpu)
    ...
    saliency_samples_bayes = calculator.compute(
        test, M=M, converter=concat_mols, train=True)
