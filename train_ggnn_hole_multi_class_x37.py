...
from parsers_multi import CSVFileParserForPair
...
from chainer_chemistry.models import NFP, SchNet, WeaveNet, RSGCN, Cosine

from setting import *
from models import GGNN
from models import GIN
from models import MLP, NTN, HolE, DistMult
# from training.extensions import ROCAUCEvaluator, PRCAUCEvaluator, F1Evaluator, AccuracyEvaluator
from training.multilabel_extensions import ROCAUCEvaluator, PRCAUCEvaluator, F1Evaluator, AccuracyEvaluator
...
class GraphConvPredictorForPair(chainer.Chain):
    ...
    def __call__(self, atoms_1, adjs_1, atoms_2, adjs_2):
        ...
        if self.mlp.__class__.__name__ == 'MLP':
            ...
            h = self.mlp(h)
            return h
        elif self.mlp.__class__.__name__ in \
            ['NTN', 'SymMLP', 'HolE', 'DistMult', 'Cosine']:
            h = self.mlp(h1, h2)
            return h
...
def get_class_num():
    ...
    label_filepath = os.path.join(KAIST_PATH, label_filename)
    labels_all = pd.read_csv(label_filepath)['General sentence structure']
    class_num = len(labels_all)
    return class_num


def parse_arguments():
    # Lists of supported preprocessing methods/models.
    method_list = ['ggnn', 'nfp', 'weavenet', 'gin', 'rsgcn']
...
def main():
    ...
    train, enc = to_multi_hot_labels(train, enc=None)
    valid = parser.parse(args.valid_datafile)['dataset']
    valid, _ = to_multi_hot_labels(valid, enc=enc)
...
if __name__ == '__main__':
    ...
    """
python train_ggnn_hole_multi_class_x37.py --method=ggnn --epoch=200 --learning-rate=0.001 --exp-shift-rate=0.5 --weight-tying=False --fp-hidden-dim=32 --conv-layers=8 --net-hidden-dims= --out=result_kaist_ggnn_epoch200_lr0.001_exp0.5_wt0_fph32_conv8_nh_hole_x37 --gpu=1 --train-datafile=dataset/kaist/ddi_train_multi.csv --valid-datafile=dataset/kaist/ddi_valid_multi.csv --sim-method=hole --augment=True
    """
