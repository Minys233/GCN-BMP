...
flags.DEFINE_string('feature', None, 'Name of feature as initialization')
...
def get_roc_score(edges_pos, edges_neg):
    ...
    acc_score = metrics.accuracy_score(labels_all, preds_all >= 0.5)
    f1_score = metrics.f1_score(labels_all, preds_all >= 0.5)
...
class DenseGCNModel(object):
    def __init__(self, placeholders, num_features, name):
        self.name = name
        self.inputs = placeholders['features']
        self.input_dim = num_features
        self.adj = placeholders['adj']
...
if FLAGS.feature is None:
    ...
    with open(feature_dict_filepath, 'rb') as pkl_reader:
        feature_dict = pickle.load(pkl_reader)
    ...
    cid2smiles = dict(zip(drug_list_df['cid'], drug_list_df['smiles']))
    feature_list = list()
    for index in range(num_nodes):
        cid = ind2cid[index]
        smiles = cid2smiles[cid]
        feature_vec = feature_dict[smiles]
...
if FLAGS.feature is None:
    ...
    model = DenseGCNModel(placeholders, num_features, name='ddi_dense_gcn')
