#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 3/16/2019 7:35 PM
# @Author  : chinshin
# @FileName: ggnn_dev.py
...
class GGNN(chainer.Chain):
    ...
    def __init__(self, out_dim, hidden_dim=16,
                 ...
                 weight_tying=True, output_atoms=True,
                 ):
    ...
    def att_readout(self, h, h_list, step=0):
        # --- Readout part ---
        index = step if self.concat_hidden else 0
        h_concat = functions.concat(h_list, axis=2)
        # i_layer_out: (minibatch, atom, ch)
        g = functions.sigmoid(
            self.i_layers[index](h_concat)) \
            * self.j_layers[index](h)
        g = functions.sum(g, axis=1)  # sum along atom's axis
        return g
    ...
    def get_atom_array(self, step=-1):
        assert len(self.atoms_list) > 0
        return self.atoms_list[step]

    def get_g_list(self):
        assert len(self.g_vec_list) > 0
        return self.g_vec_list