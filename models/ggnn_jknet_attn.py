...
class GGNN(chainer.Chain):
    ...
    def __init__(self, out_dim, hidden_dim=16,
        ...
        with self.init_scope():
            ...
            self.aggr = AttnAggregator(n_layers, n_layers, 1)
...
class AttnAggregator(chainer.Chain):
    ...
    def __init__(self, left_size, right_size, out_size):
        ...
        with self.init_scope():
            self.bilinear_layer = links.Bilinear(left_size, right_size, out_size)
    ...
    def compute_attention(self, query, key):
        ...
        query = functions.broadcast_to(query, shape=(n_layes, mb * atom * hidden_dim))
        query = functions.transpose(query, axes=(1, 0))
        key = functions.reshape(key, shape=(n_layes, mb * atom * hidden_dim))
        key = functions.transpose(key, axes=(1, 0))

        # (n_layes, 1)
        # energy = functions.matmul(key, query, transb=True)
        # energy = energy / np.sqrt(hidden_dim)

        energy = self.bilinear_layer(key, query)
