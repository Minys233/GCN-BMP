...
class GlobalCoattention(chainer.Chain):
    def __init__(self, hidden_dim, out_dim,
        ...
        with self.init_scope():
            self.att_layers = chainer.ChainList(
                *[L.Linear(3 * hidden_dim, out_dim) for _ in range(n_entities)]
            )
    ...
    def compute_attention(self, initial_query, another_query, key, focus):
        """
        :param initial_query: with shape of (mb, N, hidden_dim)
        :param another_query: with shape of (mb, hidden_dim)
        ...
        # another_query: (mb, 1, hidden_dim)
        another_query = F.expand_dims(another_query, axis=1)
        # another_query: (mb, N, hidden_dim)
        another_query = F.tile(another_query, reps=(1, N, 1))
        # another_query: (mb * N, hidden_dim)
        another_query = F.reshape(another_query, shape=(mb * N, hidden_dim))

        # initial_query: (mb * n, hidden_dim)
        initial_query = F.reshape(initial_query, shape=(mb * N, hidden_dim))

        # key: (mb * N, hidden_dim)
        key = F.reshape(key, shape=(mb * N, hidden_dim))

        # energy: (mb * N, out_dim)
        energy = F.sigmoid(att_layer(F.concat((key, another_query, initial_query), axis=-1)))
        # energy: (mb, N, out_dim)
