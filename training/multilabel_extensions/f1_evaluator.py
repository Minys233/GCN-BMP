...
class F1Evaluator(BatchEvaluator):
    ...
    def f1_score(self, y_total, t_total,
        ...
        precision_arr = numpy.zeros(shape=(num_clses, ), dtype=numpy.float32)
        recall_arr = numpy.zeros(shape=(num_clses), dtype=numpy.float32)
        for cls in range(num_clses):
            precision = metrics.precision_score(
                y_true=t_total[:, cls], y_pred=y_total[:, cls],
            ...
            precision_arr[cls] = precision
            recall_arr[cls] = recall

        macro_precision = precision_arr.mean()
        macro_recall = recall_arr.mean()
