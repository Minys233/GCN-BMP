...
class ROCAUCEvaluator(BatchEvaluator):
    ...
    def roc_auc_score(self, y_total, t_total,
        ...
        #     y_true=t_total, y_score=y_total,
        #     average=average, sample_weight=sample_weight,
        #     # max_fpr=max_fpr
        # )

        num_clses = y_total.shape[1]
        roc_auc_arr = numpy.zeros(shape=(num_clses, ), dtype=numpy.float32)
        for cls in range(num_clses):
            roc_auc = metrics.roc_auc_score(
                y_true=t_total[:, cls], y_score=y_total[:, cls],
                average=average, sample_weight=sample_weight
            )
            roc_auc_arr[cls] = roc_auc
