...
class PRCAUCEvaluator(BatchEvaluator):
    ...
    def prc_auc_score(self, y_total, t_total,
        ...
        #     t_total, y_total, pos_label=pos_label, sample_weight=sample_weight)
        # prc_auc = metrics.auc(recall, precision)

        num_clses = y_total.shape[1]
        prc_auc_arr = numpy.zeros(shape=(num_clses, ), dtype=numpy.float32)
        for cls in range(num_clses):
            precision, recall, _ = metrics.precision_recall_curve(
                y_true=t_total[:, cls], probas_pred=y_total[:, cls],
                pos_label=pos_label, sample_weight=sample_weight,
            )
            prc_auc = metrics.auc(recall, precision)
            prc_auc_arr[cls] = prc_auc

        prc_auc = prc_auc_arr.mean()
        # prc_auc = metrics.average_precision_score(
        #     t_total, y_total, average=average, sample_weight=sample_weight
        # )
