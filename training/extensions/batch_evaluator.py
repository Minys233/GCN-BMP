...
from sklearn import metrics as perf
...
class BatchEvaluator(Evaluator):
    ...
    def evaluate(self):
        ...
        y_total = cupy.concatenate(y_total) if self.device >= 0 else numpy.concatenate(y_total)
        y_total = sigmoid(y_total)
        t_total = cupy.concatenate(t_total) if self.device >= 0 else numpy.concatenate(t_total)
